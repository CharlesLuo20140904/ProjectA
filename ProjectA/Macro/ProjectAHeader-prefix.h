...
#ifndef ProjectAHeader_prefix_h
#define ProjectAHeader_prefix_h
#import "ProjectAConst.h"
#import "ProjectAUrls.h"
...
// 获取沙盒下directory目录的路径
//方式一：
#define KDocFolder [NSHomeDirectory() stringByAppendingPathComponent:@"Documents"]
//方式二：
...
//获取沙盒下tmp目录的路径
#define KTmpPath [NSHomeDirectory() stringByAppendingPathComponent:@"tmp"]
//创建文件路径 directory
#define KDocFilePath(s) [KDocPath stringByAppendingPathComponent:s]
//创建文件路径 Cache
#define KCacheFilePath(s) [KCachePath stringByAppendingPathComponent:s]
//创建文件路径 tmp
#define KTmpFilePath(s) [KTmpPath stringByAppendingPathComponent:s]

// 保存到plist文件
#define KWriteToPlist(dic,s) [dic writeToFile:filePath(s) atomically:YES];
// 读取plist文件到字典
#define KReadToPlist(s) [[[NSMutableDictionary alloc] initWithContentsOfFile:filePath(s)] mutableCopy]


//MARK: ------ 获取字体的size
// 计算文本的尺寸
#define KTextSizeG(text, fontSize, width) [text boundingRectWithSize:CGSizeMake(width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:fontSize]} context:nil].size;

#define KTextSize(text, font, width) [text boundingRectWithSize:CGSizeMake(width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:font} context:nil].size;

#define KTextSize_IOS6 (text, font , width) [text sizeWithFont:font constrainedToSize:CGSizeMake(width, MAXFLOAT)];


//MARK: ------ 颜色
#define KRGBValue(rgbValue)   [UIColor colorWithRed:((float)((rgbValue & 0xFF0000) >> 16))/255.0f green:((float)((rgbValue & 0xFF00) >> 8))/255.0f blue:((float)(rgbValue & 0xFF))/255.0f alpha:1.0f]
...
//刘海屏812
#define KNavH (IS_IPHONE && SCREEN_MAX_LENGTH >= 812.0?88:64)
//刘海屏比之前的导航栏高了24
#define KNavH_Add (IS_IPHONE && SCREEN_MAX_LENGTH >= 812.0?24:0)
//刘海屏tabbar高度
#define KTabH (IS_IPHONE && SCREEN_MAX_LENGTH >= 812.0?82:49)
//刘海屏比之前的tabbar多了34的底部宽度
#define KTab_Add (IS_IPHONE && SCREEN_MAX_LENGTH >= 812.0?34:0)
//英文键盘的高度
